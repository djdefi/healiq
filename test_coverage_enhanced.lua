...
local function runEnhancedTests()
    ...
    if HealIQ.UI then
        ...
        if HealIQ.UI.SetPosition then
            ...
            assert_test(positionSuccess, "UI: SetPosition handles coordinates", "SetPosition failed")
        end
        ...
        if HealIQ.UI.CreateMainFrame then
            ...
            assert_test(frameSuccess, "UI: CreateMainFrame handles errors", "Frame creation failed under error simulation")
        end
    end
    ...
    if HealIQ.Engine then
        ...
        if HealIQ.Engine.GetCurrentSuggestion then
            errorSimulation.simulateErrors = true
            local engineSuccess, suggestion = pcall(function()
                return HealIQ.Engine:GetCurrentSuggestion()
            end)
            errorSimulation.simulateErrors = false

            if engineSuccess and suggestion then
        ...
        if HealIQ.Engine.ForceUpdate then
            ...
            assert_test(updateSuccess, "Engine: ForceUpdate executes", "ForceUpdate failed")
        end

        if HealIQ.Engine.EvaluateRules then
            errorSimulation.simulateErrors = true
            local rulesSuccess = pcall(function()
                HealIQ.Engine:EvaluateRules()
            end)
            errorSimulation.simulateErrors = false
            assert_test(rulesSuccess, "Engine: EvaluateRules handles errors", "EvaluateRules failed under error simulation")
        end
    end
    ...
    if HealIQ.Config then
        ...
        if HealIQ.Config.HandleSlashCommand then
            local commandSuccess = pcall(function()
                HealIQ.Config:HandleSlashCommand("version")
                HealIQ.Config:HandleSlashCommand("help")
                HealIQ.Config:HandleSlashCommand("toggle")
            end)
            assert_test(commandSuccess, "Config: HandleSlashCommand works", "Command handling failed")
        end
    ...
    if HealIQ.Tracker then
        ...
        if HealIQ.Tracker.IsSpellKnown then
            errorSimulation.simulateErrors = true
            local trackerSuccess, isKnown = pcall(function()
                return HealIQ.Tracker:IsSpellKnown("Rejuvenation")
            end)
            errorSimulation.simulateErrors = false

            if trackerSuccess then
        ...
        if HealIQ.Tracker.OnEvent then
            ...
            assert_test(eventSuccess, "Tracker: OnEvent handles events", "Event handling failed")
        end
    end
    ...
    if HealIQ.Logging then
        ...
        if HealIQ.GenerateDiagnosticDump then
            local dumpSuccess, dump = pcall(function()
                return HealIQ:GenerateDiagnosticDump()
            end)

            if dumpSuccess and dump then
    ...
    for _, component in ipairs(componentTests) do
        local componentSuccess = pcall(component.test)
