...
local function initializeHealIQ()
    ...
    if type(HealIQ) ~= "table" then
        ...
        HealIQ = _G.HealIQ
        if type(HealIQ) ~= "table" then
            print("HealIQ Error: OffensiveRules.lua loaded before Core.lua - addon not initialized")
            return nil
        end
    end
...
if success and result then
    ...
    print("HealIQ Error: Failed to initialize OffensiveRules.lua - " .. tostring(result or "unknown error"))
    -- Minimal fallback
